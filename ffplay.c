...
typedef struct VideoState {
    ...
    unsigned int audio_buf_size; /*  音频缓冲区 in bytes */
    unsigned int audio_buf1_size; // 音频缓冲大小1
    int audio_buf_index; /* 音频缓冲索引 in bytes */
    int audio_write_buf_size; //音频缓冲索引
    int audio_volume; // 音量
    int muted; // 是否静音
    ...
    enum ShowMode {                
        SHOW_MODE_NONE = -1,        // 显示类型
        SHOW_MODE_VIDEO = 0,        // 无显示
        SHOW_MODE_WAVES,            // 显示视频
        SHOW_MODE_RDFT,             // 显示波浪，音频
        SHOW_MODE_NB                // 自适应滤波器
    } show_mode;
...
static AVInputFormat *file_iformat; // 输入文件格式
static const char *input_filename;// 输入文件名称
static const char *window_title;// 标题
static int default_width  = 640;// 视屏默认长度
static int default_height = 480;// 视屏默认宽度
static int screen_width  = 0;// 屏幕长度
static int screen_height = 0;// 屏幕宽度
...
//decoder_decode_frame pktqueue中获取frame
...
//初始化好每个码流的AVCodec和AVCodecContext
//创建线程 
static int stream_component_open(VideoState *is, int stream_index)
{

    /*
        开启对应的线程之前，会初始化好每个码流的AVCodec和AVCodecContext
...
// read_thread 作为数据读取线程

/*
1.初始化AVformatContext和AVStream和VideoState，
2.1解复用处理，读取pkt，放入queue
2.2创建解码线程 stream_component_open
...
static int read_thread(void *arg)
{
    ...
    //解复用处理 
    for (;;) {
        if (is->abort_request)//判断是否退出播放器 如果abort_request=0 则关闭。
            break;
...
//      of the seek_pos/seek_rel variables
        ...
        //判断数据包是否读取成功。如果没成功，则判断读取的结果是否AVERROR_EOF，即结尾标志。如果到了结尾，则入队一个空的数据包。

        if (ret < 0) {
            //如果读取出错，则直接退出读数据包流程。如果都不是，则判断解码器中的待解码数据包、待输出帧是否存在数据，
            //如果都不存在数据，则判断是否跳转至起始位置还是判断是否自动退出，或者是继续下一轮读数据包流程。
...
/*
    VideoState进行初始化
    解码数据队列(PacketQueue)初始化
    解码后数据队列(FrameQueue)初始化
    时间钟(Clock)等的初始化
    开启read_thread
*/
